...
```{r hill numbers}
h2 <- rep4$data[!(names(rep4$data) %in% c('20-5','23-1','21-1'))] #%>% repSample() # can downsample, remove 21-1 optional

# hill 1-10
# repDiversity(h2,'hill',.max.q=10,.min.q=1) %>% 
#   separate(Sample,c('patient','timepoint')) %>% add_clinical()
# hill 1,2,4,10,50
...
filter(h2,timepoint %in% c('1','h')) %>% #set timepoints
  group_by(clinical,Q) %>%
...
# 1 should be shannon, 2 is inv.simp
```
...
limitSize <- 500 #min clones to use?
